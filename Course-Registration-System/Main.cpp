...
void readStudents() {
    ...
    while (getline(file, line)) {
        ...
        if (!data.empty() && data.front() != "") {
            IDs = split(data.front(), '&'), data.pop();   // get registered courses IDs
            grades = split(data.front(), '&'), data.pop();  // get course grade
            while (!IDs.empty() && IDs.front() != "" && !grades.empty() && grades.front() != "") {
...
bool mainMenu(int choice) {
    ...
    if (choice == 1) {
        ...
        if (choice == 1) {
            ...
            student.second->second.updateGPA();
...
bool signUpAsStudent() {
    ...
    nationalID = Handleable::handlingNumbers(Handleable::emptyString(nationalID, "National ID"), "National ID");
    cout << "Enter Your Telephone Number: ";
    telephoneNumber = Handleable::handlingNumbers(readLine(), "Telephone Number");
    ...
    Student student(ID, ID + "@cis.asu.edu.eg", password, name, nationalID, telephoneNumber, address, nationality, 0.0, 1, 0);
